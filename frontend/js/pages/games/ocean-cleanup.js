// Ocean Cleanup Challenge Game Logic
class OceanCleanupGame {
    ...
    activatePowerUp(type) {
        ...
        powerUpElement.innerHTML = `<i class="fas fa-${type === 'speed' ? 'bolt' : type === 'points' ? 'star' : 'shield'}"></i> ${type}`;
    ...
    handleGameClick(e) {
        ...
        if (clickedCreature) {
            this.loseLife();
            this.showFloatingText('-1 Life', e.clientX - this.gameArea.offsetLeft + 'px', e.clientY - this.gameArea.offsetTop + 'px', 'error');
            return;
        }

        // Missed click (slight penalty to accuracy)
        this.updateUI();
    }
    ...
    updateUI() {
        ...
        this.accuracy = this.totalClicks > 0 ? Math.round((this.correctClicks / this.totalClicks) * 100) : 0;
    }
